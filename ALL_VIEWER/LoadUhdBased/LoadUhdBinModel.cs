...
namespace LoadUhdBased.src
{
    public class LoadUhdBinModel
    {
        ...
        public LoadUhdBinModel(ModelGroup modelGroup, ModelNodeGroup mng, bool isPS4NS, Endianness endianness)
        {
        ...
        public void LoadUhdBIN(string binPath)
        {
            ...
            if (!modelGroup.Objects.ContainsKey(FileID))
            {
                ...
                try
                {
                    ...
                    uhdBIN = UhdBinDecoder.Decoder(binFile, 0, out _, isPS4NS, endianness);
                }
                catch (Exception)
                {
                }
        ...
        public static void PopulateTreatedModel(ref TreatedModel treatedModel, ref UhdBIN uhdBIN, string FileID) 
        {
            ...
            for (int im = 0; im < uhdBIN.Materials.Length; im++)
            {
                ...
                for (int i = baseIndex; i < maxIndex + 1; i++)
                {
                    ...
                    if (uhdBIN.Header.ReturnsHasEnableVertexColorsTag() && uhdBIN.Vertex_Color_Array.Length > 0)
                    {
