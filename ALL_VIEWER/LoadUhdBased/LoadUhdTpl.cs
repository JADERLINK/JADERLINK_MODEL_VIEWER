...
namespace LoadUhdBased.src
{
    public class LoadUhdTpl
    {
        ...
        public LoadUhdTpl(ModelGroup modelGroup, ModelNodeGroup mng, bool isPS4NS, Endianness endianness)
        {
        ...
        public void LoadUhdTPL(string tplPath)
        {
            ...
            if (!modelGroup.MatTexGroupDic.ContainsKey(FileID))
            {
                ...
                try
                {
                    ...
                    uhdTPL = UhdTplDecoder.Decoder(tplFile, 0, out _, isPS4NS, endianness);
                }
                catch (Exception)
                {
                }
